...
/*prints out impact safety mesage*/
...
/*Draws the firework on the comeup*/
...
void printTable(void *data)
{
	...
	if (aFWPtr->launchTime <= aFWPtr->sim->ET)printf("%X at  ( %7.2lf,    %7.2lf)'  ( %7.2lf,   %7.2lf) fps with %.3lf sec\n", aFWPtr->code, aFWPtr->X , aFWPtr->Y, aFWPtr->XV, aFWPtr->YV, aFWPtr->fuseTime);
...
void printFireworkExplode(void *data)
{
	...
	if (firework->fuseTime <= firework->sim->dt && firework->launchTime <= ((firework->sim->ET) + firework->sim->dt))
	{
		/*Check the relation between dt and fusetime*/
		newDT = firework->fuseTime;
...
/*Sort the fireworks by launchtime order*/
